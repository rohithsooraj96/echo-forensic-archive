...
export function CommandPalette({ data, open, filters, onClose, onFilter, onWorld, onConnections }: Props) {
  ...
  function choose(action: string) {
    ...
    else if (action === 'year-2023') onFilter({ ...filters, year: '2023' })
    else if (action === 'timezones') onWorld('overview')
    else if (action === 'transaction-types') { onWorld('transactions'); onFilter({ ...filters, dataset: 'household' }) }
    onClose()
  }
  return <div className="modal-scrim" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="command-palette" data-testid="command-palette" role="dialog" aria-modal="true" aria-labelledby="command-title"><div className="palette-head"><div><span className="palette-icon"><Command size={16} /></span><div><p className="eyebrow">COMMAND INDEX</p><h2 id="command-title">Find in the archive</h2></div></div><button data-testid="command-palette-close" onClick={onClose} aria-label="Close command palette"><X size={18} /></button></div><label className="palette-input"><Search size={17} /><input ref={input} data-testid="command-palette-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search an artist, trace, command..." /><kbd>ESC</kbd></label><div className="palette-results" role="listbox" aria-label="Search results">{results.length ? results.map((result) => <button className="palette-result" data-testid={`command-result-${result.action}`} key={`${result.label}-${result.action}`} onClick={() => choose(result.action)}><span className="result-icon"><Sparkles size={15} /></span><span><b>{result.label}</b><small>{result.detail}</small></span><span className="result-arrow">↗</span></button>) : <p className="empty-state">NO MATCHES / TRY A DIFFERENT TRACE</p>}</div><div className="palette-footer"><span>TYPE TO SEARCH</span><span><kbd>↑↓</kbd> NAVIGATE <kbd>↵</kbd> SELECT</span><ActionButton variant="ghost" data-testid="palette-scan-connections" onClick={() => choose('connections')}>SCAN CONNECTIONS <Sparkles size={14} /></ActionButton></div></section></div>
}
