...
export function CircadianMatrix({ data, selectedHour, onSelect }: Props) {
  ...
  const spotify = data.spotifyHourly.map((item) => item.events)
  const household = data.householdHourly.map((item) => item.events)
  const india = data.indiaHourly.map((item) => item.events)
  const peak = useMemo(() => ({ spotify: maxOf(spotify), household: maxOf(household), india: maxOf(india) }), [spotify, household, india])
  ...
  function select(hourIndex: number) { setHoveredHour(hourIndex); onSelect(hourIndex) }
