...
export function ArchiveShell({ data }: Props) {
  ...
  const selectWorld = (next: WorldId) => { setWorld(next); logTelemetry('view_open', next); ping(next === 'transactions' ? 260 : 520) }
  const openPalette = () => { setPaletteOpen(true); logTelemetry('search', world) }
  const closePanels = () => { setPaletteOpen(false); setCaseOpen(false); setHelpOpen(false); setSelectedNode(null) }
  const findConnection = () => { const node = data.connections[Math.floor(Math.random() * data.connections.length)]; if (!node) return; const destination = node.datasets[0] === 'spotify' ? 'listening' : node.datasets[0] === 'household' ? 'spending' : 'transactions'; setSelectedNode(null); setFlightNodeId(node.id); setWorld('overview'); logTelemetry('view_open', 'connection-flight'); connectionSweep(); window.setTimeout(() => { setFlightNodeId(null); setSelectedNode(node); setWorld(destination); }, 1150) }
  const bookmark = (item: BookmarkType) => { setBookmarks((items) => items.some((existing) => existing.id === item.id) ? items : [...items, item]); setCaseOpen(true); logTelemetry('case_export', world); ping(740) }
  const updateFilters = (next: FilterState) => { setFilters(next); logTelemetry('filter_change', world) }
  const exportCurrent = () => { exportCaseFile(bookmarks, filters, data.manifest); logTelemetry('case_export', world) }
