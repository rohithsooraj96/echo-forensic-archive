...
export function TelemetryBar({ visible, indexed, range, entropy, muted, onMute, onHelp, onExport }: Props) {
  return <footer className="telemetry-bar" aria-label="Forensic telemetry">
    <div className="sr-only" aria-live="polite">ECHO live archive status: {formatNumber(indexed)} records indexed; {formatNumber(visible)} visible traces; active filter {range}; entropy index {Math.round(entropy * 100)} percent.</div>
    ...
    <div className="telemetry-actions"><button aria-label={muted ? 'Enable archive audio' : 'Mute archive audio'} aria-pressed={!muted} onClick={onMute}>{muted ? <VolumeX size={15} /> : <Volume2 size={15} />}</button><button aria-label="Open keyboard shortcuts" onClick={onHelp}><Keyboard size={15} /></button><button aria-label="Export case file" onClick={onExport}><Download size={15} /></button><CircleHelp size={14} className="telemetry-help" /></div>
  </footer>
}
